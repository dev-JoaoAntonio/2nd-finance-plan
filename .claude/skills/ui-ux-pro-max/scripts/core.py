...
# Auto-discover every stack CSV shipped in data/stacks/ (vue, react, nextjs, ...).
# Local patch: upstream v2.5.0 hardcoded only react-native here even though all
# 13 stack CSVs ship in data/stacks/, which broke `--stack vue`. Scanning the
# directory keeps STACK_CONFIG in sync with whatever stack files are present.
_STACKS_DIR = DATA_DIR / "stacks"
if _STACKS_DIR.exists():
    STACK_CONFIG = {
        csv_file.stem: {"file": f"stacks/{csv_file.name}"}
        for csv_file in sorted(_STACKS_DIR.glob("*.csv"))
    }
else:
    STACK_CONFIG = {
